...
<div class="content-header">
    <div class="container-fluid">
      <div class="row mb-2">
        <div class="col-sm-6">
          <h1 class="m-0">{{ __('List contacts') }}</h1>
        </div><!-- /.col -->
        <div class="col-sm-6">
          <ol class="breadcrumb float-sm-right">
            <li class="breadcrumb-item"><a href="#">{{ __('Home') }}</a></li>
            <li class="breadcrumb-item active">{{ __('List contacts') }}</li>
          </ol>
        </div><!-- /.col -->
      </div><!-- /.row -->
    </div><!-- /.container-fluid -->
...
<section class="content">
    <div class="container-fluid">
                ...
                <div class="card-header">
                    <div class="row">
                        ...
                        <div class="col-lg-8 group-btn text-right">
                            <button type="button" class="btn btn-primary add_contact" data-container=".contact_modal"
                            data-href="{{ route('contacts.create') }}"><i class="fa fa-plus"></i> {{ __('Add contact') }}</button>
                        </div>
                    </div>
                </div>
                <!-- /.card-header -->
                <div class="card-body">
                    <table id="contact_table" class="table table-bordered table-hover ajax_view">
                    <thead>
                        <tr>
                            <th style="width: 14%">{{ __('Image') }}</th>
                            <th style="width: 24%">{{ __('Name') }}</th>
                            <th style="width: 50%">{{ __('Link') }}</th>
        ...
        <div class="modal fade contact_modal" id="contact_modal" tabindex="-1" role="dialog"></div>
    </div><!-- /.container-fluid -->
</section>
<!-- /.content -->
@endsection
...
<script>
    ...
    $(document).on('click', '.add_contact', function(e) {
        e.preventDefault();
        $('div.contact_modal').load($(this).attr('data-href'), function() {
            $(this).modal('show');
        });
    });
